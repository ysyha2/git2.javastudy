...
public class D09_CollectEmail {
	...
	public static void email_check(String str) {
		Pattern pattern = Pattern.compile("\\w+@\\w+(\\.\\w+)+");
		Matcher matcher = pattern.matcher(str);
		ArrayList<String> email_collect = new ArrayList<>();
		while(matcher.find()) {
			email_collect.add(matcher.group());
		}
		System.out.println("총 이메일 수는 " + email_collect.size() + "입니다");
		
		for(int i = 0; i < email_collect.size(); i++) {
			for(int j = i + 1; j < email_collect.size(); j++) {
				if(email_collect.get(i).equals(email_collect.get(j))) {
					System.out.println("중복되는 이메일은 " + email_collect.get(i) + "입니다");
				}
			}
		}
	}

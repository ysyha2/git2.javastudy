...
//			comparator = (Comparator<Info>) comparators[column].getConstructor(Integer.class).newInstance(ordering);
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		for(HashMap<String, Info> group : phonebook.values()) {
			info_set.addAll(group.values());
//				System.out.println(info.getClass().getDeclaredFields()[column].getName());
		}
